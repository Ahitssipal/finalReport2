...
- **그림 그리기 영역**: 마우스를 사용하여 특정 영역에 그림을 그릴 수 있다.
- **저장 및 불러오기**: 그린 이미지를 파일로 저장하고 파일에서 이미지를 불러올 수 있다.
- **그림 지우기**: 그림 그리기 영역을 지울 수 있다.
- **종료**: 응용 프로그램을 종료.
- **기능 버튼**: 외곽선 검출 및 무게 중심 계산 등의 사용자 정의 기능을 수행한다.

버튼을 클릭하여 다양한 작업을 수행함:
  - **Save**: 현재 그림을 파일로 저장.
  - **Load**: 파일에서 이미지를 불러옴.
  - **Clean**: 그림 그리기 영역을 처음으로 되돌림.
  - **Exit**: 응용 프로그램 종료.
  - **Contours**: 그림에서 외곽선을 검출하여 표시.
  - **Center**: 가장 큰 외곽선의 무게 중심을 계산 및 출력.
...
그림을 그레이스케일로 변환하고 외곽선을 검출하여 검출된 외곽선의 수를 출력한다.
```
...
외각선 내의 이미지 객체의 무게중심을 계산하고, 이를 그림 영역의 백분율로 출력한다.
```
